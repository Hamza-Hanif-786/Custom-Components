...
import React, { useState, useCallback } from 'react'; // Import useCallback if using it here too
// import Rating from './Rating'; // Adjust path

export function RatingExample() {
  const [currentRating, setCurrentRating] = useState(3.5);

  // Example of memoizing the callback passed to the Rating component
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <Rating
        ...
        onChange={handleRatingChange} // Pass the memoized callback
        size={32}
        activeColor="darkorange"
      />
      <p>Selected Rating: {currentRating}</p>

      <h2>Read-Only Rating (Float)</h2>
      <Rating
        initialValue={4.7}
        readonly={true}
        size={20}
